...
const PLANTILLAS_MOCK: Plantilla[] = [
  {
    ...
    contenido: 'Hola {{nombre}}, te escribo para dar seguimiento a tu consulta sobre {{producto}}. ¿Te gustaría que agendemos una llamada? ',
    tipo: 'WhatsApp',
  ...
  },
  {
    ...
    contenido: 'Hola {{nombre}}, tenemos una oferta vigente para {{producto}} con un beneficio especial hasta {{fecha_limite}}.',
    tipo: 'Email',
    variables: ['nombre', 'producto', 'fecha_limite'],
  },
  {
    id: 3,
    nombre: 'Cierre amable',
    contenido: 'Gracias por tu tiempo, {{nombre}}. Si te interesa retomar la conversación, estaré atento para ayudarte.',
    tipo: 'WhatsApp',
    variables: ['nombre'],
  },
];

const applyTemplateVariables = (template: string, values: Record<string, string>): string => {
...
export const InboxVendedor: React.FC<InboxVendedorProps> = ({ vendedorId, vendedorNombre }) => {
  ...
  const handleAplicarPlantilla = () => {
    ...
    const contenido = applyTemplateVariables(plantillaSeleccionada.contenido, variables);
  ...
  const descargarPDF = (conversacion: Conversacion) => {
    try {
      const lines: string[] = [
        'Reporte de Conversacion',
        `ID: ${conversacion.id}`,
        `Canal: ${conversacion.canal}`,
        `Contacto: ${conversacion.contactoNombre || 'Desconocido'}`,
        `Estado: ${conversacion.estado || 'activo'}`,
        `Fecha: ${new Date(conversacion.fechaHora).toLocaleString('es-ES')}`,
        '',
        'Mensajes:',
      ];

      (conversacion.mensajes || []).forEach((msg) => {
        lines.push(`[${new Date(msg.fechaHora).toLocaleString('es-ES')}] ${msg.remitente}: ${msg.contenido}`);
      });
      ...
      link.href = url;
      link.download = `conversacion_${conversacion.id}_${Date.now()}.txt`;
      link.click();
